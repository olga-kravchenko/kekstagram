  ...
  const getRandomComments = (quantity) => {
    ...
    for (let i = 0; i < quantity; i++) {
      comments.push({
        ...
        message: MESSAGES[window.util.getRandomNumber(MIN_ARRAY_INDEX, MESSAGES.length)],
        name: USER_NAMES[window.util.getRandomNumber(MIN_ARRAY_INDEX, USER_NAMES.length)],
      });
    }
    return comments;
  };
  ...
  const getPhotos = (quantity) => {
    ...
    for (let i = 0; i < quantity; i++) {
      photos.push({
        id: i,
        url: `photos/${i + 1}.jpg`,
        description: PHOTO_DESCRIPTIONS[window.util.getRandomNumber(MIN_ARRAY_INDEX, PHOTO_DESCRIPTIONS.length)],
